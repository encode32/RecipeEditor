...
public class OneOfDialog extends JDialog {
	...
	private java.util.List<Oneof> oneOfListTemp = new ArrayList<Oneof>();
	/**
	 * Create the dialog.
	 */
	public OneOfDialog(JFrame parent, String title) {
		super(parent, title);
		addComponentListener(new ComponentAdapter() {
			...
			public void componentShown(ComponentEvent e) {
				...
				{
					oneOfListTemp = RecipeEditor.instance.recipe.getIngredients().getOneof();
					
					ingredientList.removeAll();
					oneOfList.removeAll();
					for(Oneof oneOf : oneOfListTemp)
					{
		...
		setResizable(false);
		setBounds(100, 100, 350, 619);
		getContentPane().setLayout(new BorderLayout());
		contentPanel.setBorder(new EmptyBorder(5, 5, 5, 5));
		getContentPane().add(contentPanel, BorderLayout.CENTER);
		GridBagLayout gbl_contentPanel = new GridBagLayout();
		gbl_contentPanel.columnWidths = new int[]{100, 0, 0};
		gbl_contentPanel.rowHeights = new int[]{0, 0, 0, 0, 0, 0, 0, 0, 0, 19, 110, 0, 24, 110, 0, 0};
		...
		}
		{
			...
			gbc_ingredientList.insets = new Insets(0, 0, 5, 0);
		...
		}
		{
			...
			addListButton.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					oneOfListTemp.add(new Oneof().withList(ingredientAddList));
		...
		}
		{
			...
			gbc_oneOfList.insets = new Insets(0, 0, 5, 0);
			gbc_oneOfList.fill = GridBagConstraints.BOTH;
			gbc_oneOfList.gridwidth = 2;
			gbc_oneOfList.gridx = 0;
			gbc_oneOfList.gridy = 13;
			contentPanel.add(oneOfList, gbc_oneOfList);
		}
		{
			...
			deleteListButton.addMouseListener(new MouseAdapter() {
				@Override
				public void mouseClicked(MouseEvent e) {
					int index = oneOfList.getSelectedIndex();
					if(index != -1)
					{
						oneOfListTemp.remove(oneOfList.getSelectedIndex());
						oneOfList.remove(oneOfList.getSelectedIndex());
					}
				}
			});
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						...
						{
							RecipeEditor.instance.recipe.setIngredients(new Ingredients().withOneof(oneOfListTemp));
						}else
						{
							RecipeEditor.instance.recipe.getIngredients().setOneof(oneOfListTemp);
						}
						RecipeEditor.instance.updateOneOfData();
						RecipeEditor.instance.oneOfDialog.setVisible(false);
					}
				});
