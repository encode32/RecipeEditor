...
public class AnyDialog extends JDialog {
	...
	private java.awt.List anyList;
	private java.util.List<Any> anyListTemp = new ArrayList<Any>();
	/**
	 * 
	 * Create the dialog.
	 */
	public AnyDialog(JFrame parent, String title) {
		super(parent, title);
		addComponentListener(new ComponentAdapter() {
			@Override
			public void componentShown(ComponentEvent e) {
				if(RecipeEditor.instance.recipe.getIngredients() != null && RecipeEditor.instance.recipe.getIngredients().getAny() != null)
				{
					anyListTemp = RecipeEditor.instance.recipe.getIngredients().getAny();
					
					anyList.removeAll();
					for(Any any : anyListTemp)
					{
						anyList.add(
								...
								(any.getDifficulty() != null ? " D: " + any.getDifficulty().toString() : "") +
								(any.getRatio() != null ? " R: " + any.getRatio().toString() : "") +
								(any.getLoss() != null ? " L: " + any.getLoss().toString() : "") +
								(any.getAmount() != null ? " R: " + any.getAmount().toString() : ""));
					}
				}
			}
		});
		...
		}
		{
			...
			addButton.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					{
						Any anyTemp = new Any().withId(idField.getText());
						
						if(!cstateField.getText().isEmpty()){anyTemp = anyTemp.withCstate(cstateField.getText());}
						if(!pstateField.getText().isEmpty()){anyTemp = anyTemp.withPstate(pstateField.getText());}
						if(!materialField.getText().isEmpty()){anyTemp = anyTemp.withMaterial(materialField.getText());}
						if(!realTemplateField.getText().isEmpty()){anyTemp = anyTemp.withRealtemplate(realTemplateField.getText());}
						if(!difficultyField.getText().isEmpty()){anyTemp = anyTemp.withDifficulty(Float.valueOf(difficultyField.getText()));}
						if(!ratioField.getText().isEmpty()){anyTemp = anyTemp.withRatio(Float.valueOf(ratioField.getText()));}
						if(!lossField.getText().isEmpty()){anyTemp = anyTemp.withLoss(Float.valueOf(lossField.getText()));}
						if(!amountField.getText().isEmpty()){anyTemp = anyTemp.withAmount(Float.valueOf(amountField.getText()));}
		...
		}
		{
			...
			contentPanel.add(anyList, gbc_anyList);
		}
		{
			...
			deleteButton.addMouseListener(new MouseAdapter() {
				@Override
				public void mouseClicked(MouseEvent e) {
					int index = anyList.getSelectedIndex();
					if(index != -1)
					{
						anyListTemp.remove(anyList.getSelectedIndex());
						anyList.remove(anyList.getSelectedIndex());
					}
				}
			});
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						...
						{
							RecipeEditor.instance.recipe.setIngredients(new Ingredients().withAny(anyListTemp));
						}else
						{
							RecipeEditor.instance.recipe.getIngredients().setAny(anyListTemp);
						}
						RecipeEditor.instance.updateAnyData();
						RecipeEditor.instance.anyDialog.setVisible(false);
					}
				});
