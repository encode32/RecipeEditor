...
public class MandatoryDialog extends JDialog {
	...
	private java.awt.List mandatoryList;
	private java.util.List<Mandatory> mandatoryListTemp = new ArrayList<Mandatory>();
	/**
	 * Create the dialog.
	 */
	public MandatoryDialog(JFrame parent, String title) {
		super(parent, title);
		addComponentListener(new ComponentAdapter() {
			@Override
			public void componentShown(ComponentEvent e) {
				if(RecipeEditor.instance.recipe.getIngredients() != null && RecipeEditor.instance.recipe.getIngredients().getMandatory() != null)
				{
					mandatoryListTemp = RecipeEditor.instance.recipe.getIngredients().getMandatory();
					
					mandatoryList.removeAll();
					for(Mandatory mandatory : mandatoryListTemp)
					{
						mandatoryList.add(
								...
								(mandatory.getDifficulty() != null ? " D: " + mandatory.getDifficulty().toString() : "") +
								(mandatory.getRatio() != null ? " R: " + mandatory.getRatio().toString() : "") +
								(mandatory.getLoss() != null ? " L: " + mandatory.getLoss().toString() : "") +
								(mandatory.getAmount() != null ? " R: " + mandatory.getAmount().toString() : ""));
					}
				}
			}
		});
		...
		}
		{
			...
			addButton.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					{
						Mandatory mandatoryTemp = new Mandatory().withId(idField.getText());
						
						if(!cstateField.getText().isEmpty()){mandatoryTemp = mandatoryTemp.withCstate(cstateField.getText());}
						if(!pstateField.getText().isEmpty()){mandatoryTemp = mandatoryTemp.withPstate(pstateField.getText());}
						if(!materialField.getText().isEmpty()){mandatoryTemp = mandatoryTemp.withMaterial(materialField.getText());}
						if(!realTemplateField.getText().isEmpty()){mandatoryTemp = mandatoryTemp.withRealtemplate(realTemplateField.getText());}
						if(!difficultyField.getText().isEmpty()){mandatoryTemp = mandatoryTemp.withDifficulty(Float.valueOf(difficultyField.getText()));}
						if(!ratioField.getText().isEmpty()){mandatoryTemp = mandatoryTemp.withRatio(Float.valueOf(ratioField.getText()));}
						if(!lossField.getText().isEmpty()){mandatoryTemp = mandatoryTemp.withLoss(Float.valueOf(lossField.getText()));}
						if(!amountField.getText().isEmpty()){mandatoryTemp = mandatoryTemp.withAmount(Float.valueOf(amountField.getText()));}
						...
						mandatoryListTemp.add(mandatoryTemp);
					}
				}
			});
		...
		{
			mandatoryList = new java.awt.List();
			...
			contentPanel.add(mandatoryList, gbc_anyList);
		}
		{
			...
			deleteButton.addMouseListener(new MouseAdapter() {
				@Override
				public void mouseClicked(MouseEvent e) {
					int index = mandatoryList.getSelectedIndex();
					if(index != -1)
					{
						mandatoryListTemp.remove(mandatoryList.getSelectedIndex());
						mandatoryList.remove(mandatoryList.getSelectedIndex());
					}
				}
			});
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						...
						{
							RecipeEditor.instance.recipe.setIngredients(new Ingredients().withMandatory(mandatoryListTemp));
						}else
						{
							RecipeEditor.instance.recipe.getIngredients().setMandatory(mandatoryListTemp);
						}
						RecipeEditor.instance.updateMandatoryData();
						RecipeEditor.instance.mandatoryDialog.setVisible(false);
					}
				});
