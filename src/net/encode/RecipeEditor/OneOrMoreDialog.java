...
public class OneOrMoreDialog extends JDialog {
	...
	private java.util.List<Oneormore> oneOrMoreListTemp = new ArrayList<Oneormore>();
	/**
	 * Create the dialog.
	 */
	public OneOrMoreDialog(JFrame parent, String title) {
		super(parent, title);
		addComponentListener(new ComponentAdapter() {
			...
			public void componentShown(ComponentEvent e) {
				if(RecipeEditor.instance.recipe.getIngredients() != null && RecipeEditor.instance.recipe.getIngredients().getOneormore() != null)
				{
					...
					for(Oneormore oneOrMore : oneOrMoreListTemp)
					{
		...
		}
		{
			...
			addListButton.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					oneOrMoreListTemp.add(new Oneormore().withList(ingredientAddList));
		...
		}
		{
			...
			gbc_oneOrMoreList.fill = GridBagConstraints.BOTH;
			gbc_oneOrMoreList.insets = new Insets(0, 0, 5, 0);
			gbc_oneOrMoreList.gridwidth = 2;
			gbc_oneOrMoreList.gridx = 0;
			gbc_oneOrMoreList.gridy = 13;
			contentPanel.add(oneOrMoreList, gbc_oneOrMoreList);
		}
		{
			...
			deleteListButton.addMouseListener(new MouseAdapter() {
				@Override
				public void mouseClicked(MouseEvent e) {
					int index = oneOrMoreList.getSelectedIndex();
					if(index != -1)
					{
						oneOrMoreListTemp.remove(oneOrMoreList.getSelectedIndex());
						oneOrMoreList.remove(oneOrMoreList.getSelectedIndex());
					}
				}
			});
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						...
						{
							RecipeEditor.instance.recipe.setIngredients(new Ingredients().withOneormore(oneOrMoreListTemp));
						}else
						{
							RecipeEditor.instance.recipe.getIngredients().setOneormore(oneOrMoreListTemp);
						}
						RecipeEditor.instance.updateOneOrMoreData();
						RecipeEditor.instance.oneOrMoreDialog.setVisible(false);
					}
				});
